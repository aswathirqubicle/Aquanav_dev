...
| 6.3 | **L29** `lineTotal` | Make **tax-exclusive** everywhere; server-validate that lines sum to `subtotal`. **Project cost allocation uses the discounted line amount** — each line's share of the header discount subtracted, reusing P4b's apportionment — so `Σ project cost = Purchase Expense` net of discount. *Required by the net-into-revenue decision: expense is now net, so allocation must be net too, or every discounted invoice leaves project cost ≠ GL by the discount* |
| 6.4 | **L28** status bypass | Constrain create to `draft`; ignore a caller-supplied status |
| 6.5 | **L11** projectId | On cancellation and payment rows |
...
| **T6.5** | *(L29)* GIVEN a PO with tax converted to an invoice → WHEN approved → THEN `Σ lineTotal` = `subtotal` (tax-exclusive) and project cost matches GL `Purchase Expense` **exactly** |
| **T6.6** | GIVEN a client-supplied `lineTotal` that does not sum to `subtotal` → WHEN saved → THEN **rejected** |
| **T6.7** | GIVEN an approved PI on project X, with VAT → WHEN cancelled → THEN the reversal set mirrors **every** original line including `VAT Recoverable`, all rows carry `project_id = X` (L11), and project cost returns to its prior value |
| **T6.8** | GIVEN a supplier payment → THEN `Dr AP` / `Cr Cash/Bank`, with `project_id` set |
