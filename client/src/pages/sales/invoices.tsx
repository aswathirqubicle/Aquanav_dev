...
export default function SalesInvoicesPage() {
  ...
  const updateInvoiceMutation = useMutation({
    mutationFn: async ({ id, data, editNote }: { id: number; data: any; editNote: string }) => {
      // Same shared engine as the create path and the server. See the note there.
      const totals = docTotals(
        data.items,
        data.discountPercentage,
        data.discount,
  ...
  // Converting a quotation used to pre-fill this form directly — both documents
  // were one component. Now the quotations page navigates here with the id and
  // the quotation is loaded to pre-fill the same fields it copied before.
  //
  // It stays a pre-fill rather than a server-side conversion: the quotation is
  // only marked converted once the invoice is actually created (see
  // sales-invoices.routes.ts), so abandoning this form leaves it billable.
  const handledQuotationRef = useRef<string | null>(null);

  useEffect(() => {
    const fromQuotation = new URLSearchParams(window.location.search).get(
      "fromQuotation",
    );
    if (!fromQuotation || handledQuotationRef.current === fromQuotation) return;
    handledQuotationRef.current = fromQuotation;

    (async () => {
      try {
        const response = await apiRequest(
          `/api/sales-quotations/${fromQuotation}`,
        );
        ...
        toast({
          title: "Error",
          description:
            "Could not load that quotation. Please open it again from the quotations page.",
          variant: "destructive",
        });
      }
      // Drop the parameter once it has been handled, so a refresh or a Back
      // navigation doesn't reopen the form on a quotation already dealt with.
      // Through wouter rather than history.replaceState so its own location
      // state stays in step with the address bar.
      setLocation("/sales-invoices", { replace: true });
    })();
  }, [location]);
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
          <div>
            ...
            <p className="text-slate-600 dark:text-slate-400 mt-1">
              Raise customer invoices and record payments
            </p>
          </div>
                            ...
                            <table className="w-full min-w-[760px] table-fixed">
                              ...
                              <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                {invoiceFormData.items.map((item, index) => {
                                  ...
                                  const line = invoiceTotals.lines[index];
      ...
      >
        <DialogContent className="max-w-6xl max-h-[90vh] p-0 gap-0 overflow-hidden flex flex-col">
          {selectedInvoice ? (() => {
            ...
            // Resolved in SQL alongside the customer and project names. It used
            // to be looked up in the quotations list this page held, which only
            // ever covered the current page of that list — so the source
            // quotation silently went missing for anything not on it.
            const fromQuotation = (selectedInvoice as any).quotationNumber;
