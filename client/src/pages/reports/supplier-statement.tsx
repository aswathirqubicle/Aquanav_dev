...
export default function SupplierStatementPage() {
  ...
  const { data: suppliersResponse } = useQuery<{ data: Supplier[] }>({
    queryKey: ["/api/suppliers?limit=1000"],
  ...
  const hasActiveFilters = selectedSupplier !== "all" || dateFrom || dateTo;
  ...
  const fetchStatementData = async (
    ...
    try {
      ...
      const effectiveSupplier = filters?.supplier ?? selectedSupplier;
      ...
      if (effectiveSupplier && effectiveSupplier !== "all") {
        params.set("supplierId", effectiveSupplier);
      }
      if (effectiveDateFrom) params.set("dateFrom", effectiveDateFrom);
      if (effectiveDateTo) params.set("dateTo", effectiveDateTo);

      const response = await fetch(`/api/reports/supplier-statement?${params.toString()}`, {
        credentials: "include",
      });
      ...
      const effectiveSupplierForDesc = filters?.supplier ?? selectedSupplier;
      const isSingleSupplier =
        effectiveSupplierForDesc && effectiveSupplierForDesc !== "all";
      ...
      const transactionData = result.data.map((t: any) => {
        // AP convention: credit = invoice (increases payable), debit = payment/reversal
        ...
        return {
          ...
          // For a single-supplier view the supplier name adds no value —
          // show the actual GL description instead
          description: isSingleSupplier
            ? t.description
            : t.supplierName || t.description,
          debit: t.debit,
          credit: t.credit,
  ...
  const clearFilters = () => {
    ...
    setDateTo("");
    setCurrentPage(1);
    fetchStatementData(1, { supplier: "all", dateFrom: "", dateTo: "" });
  };
  ...
  const exportToCSV = () => {
    ...
    a.href = url;
    a.download = `supplier-statement-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
  };
    ...
    <div className="container mx-auto py-6 px-4 sm:px-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 gap-4">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h1 className="text-xl sm:text-2xl font-bold">Supplier Statement of Account</h1>
            <p className="text-sm text-muted-foreground">View supplier invoices and payment transactions</p>
          </div>
        </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 items-end">
              <div className="w-full sm:w-[220px]">
                ...
                <Autocomplete
                  ...
                  onValueChange={(v) => {
                    setSelectedSupplier(v || "all");
                    setCurrentPage(1);
                    fetchStatementData(1, { supplier: v || "all", dateFrom, dateTo });
                  }}
              ...
              <div className="w-full sm:w-auto">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setCurrentPage(1);
                    fetchStatementData(1, { supplier: selectedSupplier, dateFrom: e.target.value, dateTo });
                  }}
              ...
              <div className="w-full sm:w-auto">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setCurrentPage(1);
                    fetchStatementData(1, { supplier: selectedSupplier, dateFrom, dateTo: e.target.value });
                  }}
                  className="w-full sm:w-[150px]"
                />
              </div>
